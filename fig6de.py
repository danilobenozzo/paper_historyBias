...
if __name__=='__main__':
    ...
    filename_decoding = 'results_decoding_setRandomStateNew_noGridS_lSVC_previous_trial_S2oneTrialBack_3classes_tws_m400S11400_length200_step25_min10trialPerClass_localnTest_400trialPerClass_10rep_area1_high1Hz'
    filename_null_dist = 'results_decoding_setRandomStateNew_globalNullDist_onlyPositiveLabel_noGridS_previous_trial_S2oneTrialBack_3classes_tws_m400S11400_length200_step25_min10trialPerClass_localnTest_400trialPerClass_1000rep_area1_high1Hz'
    ...
    filename_decoding = 'results_decoding_setRandomStateNew_noGridS_lSVC_previous_trial_S2oneTrialBack_3classes_tws_m400S11400_square_length200_step25_min10trialPerClass_localnTest_400trialPerClass_10rep_area1_high1Hz'
    dataset_decoding = pickle.load(open('%s%s.pickle' % (pwd_saved, filename_decoding)))
    result_decoding = np.array(dataset_decoding['result'], float)
    window_bin = dataset_decoding['window_bin']
    length = dataset_decoding['length']
    ...
    for i in range(p_val.shape[0]):
        for j in range(p_val.shape[1]):
            k = i
            if k >= mean_null_dist.shape[-1]:
                k = np.random.randint(mean_null_dist.shape[-1])
            acc_ij = np.mean(mean_acc_decoding[:, i, j])
            p_val[i, j] = max(1.0/mean_null_dist.shape[0], ((mean_null_dist[:,k] > acc_ij).sum()+1) / (float(mean_null_dist.shape[0])+1))
